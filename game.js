...
let blockRows = 3; // mengatur jumlah minimal balok kebawah
let blockMaxRows = 10; 
let blockCount = 0;
...
function resetGame() {
    ...
    // Deteksi tombol kiri dan kanan di HP
...
// Variabel gerakan paddle
let moveLeft = false;
let moveRight = false;
let playerSpeed = 5; // Kecepatan paddle

// Fungsi update untuk menggerakkan paddle dengan tombol di HP
function movePlayerMobile() {
    if (moveLeft) {
...
// Tambahkan ke fungsi update() agar paddle bergerak
function update() {
    requestAnimationFrame(update);
    if (gameOver) return;
